...
const MbtiPage = () => {
  ...
  if (result) {
    const mbtiData = mbtiMaps[result.mbtiType] || { name: result.mbtiType, desc: "", color: "from-gray-500 to-gray-400" };
    return (
      <div className="min-h-screen bg-black px-6 py-20 text-white">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-5xl font-bold mb-4 bg-gradient-to-r text-transparent bg-clip-text pb-2 from-white to-gray-400">Khám Phá Bản Thân</h1>
          </div>
          
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            {/* Type Focus */}
            <div className={`rounded-3xl p-8 bg-gradient-to-br ${mbtiData.color} bg-opacity-20 backdrop-blur`}>
              <h2 className="text-6xl font-bold mb-2 text-white drop-shadow-md">{result.mbtiType}</h2>
              <h3 className="text-2xl font-semibold mb-4 text-white/90">{mbtiData.name}</h3>
              <p className="text-white/80 leading-relaxed text-lg">{mbtiData.desc}</p>
            </div>

            {/* Radar summary */}
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col justify-between">
               <div>
                 <h2 className="text-xl text-gray-300 uppercase tracking-widest mb-6">Thành phần tính cách</h2>
                 <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                   {Object.entries(result.scores).map(([t, score]) => (
                     <div key={t} className="bg-white/5 p-3 rounded-lg flex items-center justify-between">
          ...
          {result.recommendedMajors?.length > 0 && (
            <div>
              <h3 className="mb-6 text-3xl font-bold">Ngành Học Đề Xuất Theo MBTI</h3>
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {result.recommendedMajors.map((major) => (
                  <div key={major._id} className="rounded-3xl border border-white/10 bg-white/5 p-6 transition hover:border-purple-500/50 hover:bg-white/10">
                    ...
                    <p className="text-gray-400 text-sm line-clamp-3">{major.description}</p>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="min-h-screen bg-black px-6 py-20 text-white flex flex-col">
      <div className="mx-auto w-full max-w-3xl flex-grow flex flex-col">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <p className="text-sm uppercase tracking-[0.3em] text-purple-400">MBTI Personality Test</p>
        </div>

        {!isStarted ? (
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-400">Chưa có câu hỏi MBTI trong hệ thống</h1>
          </div>
        ) : (
          <>
            {/* Progress */}
              ...
              <div className="flex flex-col gap-4 max-w-xl mx-auto w-full">
                ...
                <button
                  ...
                  className={`relative overflow-hidden group rounded-2xl p-6 text-left border transition-all duration-300
                    ${answers[currentQuestion._id]?.typeValue === currentQuestion.optionA.typeValue
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="text-lg">{currentQuestion.optionA.text}</span>
                  </div>
                </button>
                ...
                <button
                  ...
                  className={`relative overflow-hidden group rounded-2xl p-6 text-left border transition-all duration-300
                    ${answers[currentQuestion._id]?.typeValue === currentQuestion.optionB.typeValue
                      ? "bg-indigo-600/20 border-indigo-500 text-white" 
                      : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/20"}
                  `}
                >
                  <div className="flex items-center">
                    ...
                    <span className="text-lg">{currentQuestion.optionB.text}</span>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-white/10 pt-6">
              ...
              {isAllAnswered ? (
                ...
                <button 
                  ...
                  disabled={currentIndex === totalQuestions - 1 || !answers[currentQuestion._id]}
