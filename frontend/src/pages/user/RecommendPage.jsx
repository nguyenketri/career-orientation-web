...
const RecommendPage = () => {
  ...
  const handleRecommend = async (isLoadMore = false) => {
    ...
    try {
      setError("");
      if (!isLoadMore) {
        // Không set result = null để tránh cảm giác bị "đơ" hoặc mất dữ liệu khi đang load
        setLoading(true);
      } else {
        setLoadingMore(true);
      }
  ...
  // Tự động cập nhật kết quả khi bộ lọc thay đổi (nếu đã có kết quả ban đầu)
  useEffect(() => {
    if (result) {
                ...
                {[
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        setFilters((prev) => ({ ...prev, location: loc.value }))
                      }
            ...
            <div className="mt-10 p-5 bg-slate-900 rounded-2xl text-white relative overflow-hidden group">
              <div className="relative z-10">
                ...
                <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                  Bạn chưa biết chọn ngành nào? Hãy để AI phân tích tính cách và
                  điểm số của bạn.
                </p>
                <button
